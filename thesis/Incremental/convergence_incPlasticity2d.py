...
def simulate_el(degree, cuts, quad_args):
    geo_args = {
        "name": "SQ",
        "degree": degree * np.ones(3, dtype=int),
    ...
    internal_vars = problem.solve_elastoplasticity(
        displacement, external_force, save_plastic_vars=True
    )
...
RUNSIMU = False
FOLDER2RESU = f"{FOLDER2RESU}/pls2d/"
...
if RUNSIMU:
    ...
    for k, step in enumerate(selected_steps_list):
        ...
        problem.part.postprocessing_dual(
            ...
            folder=FOLDER2RESU,
            fields={
                "plastic_equivalent": plastic_eq_quadpts,
    ...
    for quadrule, quadtype in zip(["gs"], ["leg"]):
        ...
        for i, degree in enumerate(deg_list):
            for j, cuts in enumerate(cut_list):
                ...
                for k, step in enumerate(selected_steps_list):
                    errorL2_list[k, i, j], _ = problem.norm_of_error(
                        displacement[:, :, step],
                        norm_args={
                            "type": "L2",
                            "part_ref": part_ref,
                    ...
                    errorH1_list[k, i, j], _ = problem.norm_of_error(
                        displacement[:, :, step],
                        norm_args={
                            "type": "H1",
                            "part_ref": part_ref,
                ...
                np.save(
                    f"{FOLDER2DATA}abserror_pls2d_L2_{quadrule}_{str(quadtype)}.npy",
                    errorL2_list,
                )
                np.save(
                    f"{FOLDER2DATA}abserror_pls2d_H1_{quadrule}_{str(quadtype)}.npy",
                    errorH1_list,
                )
...
for k, step in enumerate(selected_steps_list):
    for error_name in ["H1"]:
        ...
        for quadrule, quadtype, plotopt in zip(
            ["gs", "wq"], ["leg", 2], [CONFIGLINE_IGA, CONFIGLINE_WQ]
        ):
            error_list = np.load(
                f"{FOLDER2DATA}abserror_pls2d_{error_name}_{quadrule}_{str(quadtype)}.npy"
            )
            ...
            for i, degree in enumerate(deg_list):
                ...
                if quadrule == "gs":
                    ax.loglog(
                        nbel_list,
                        error_list[k, i, :],
                        # label=rf"Gauss quad. $p={degree}$",
                        color=color,
                        **plotopt,
                    )
                    slope = round(
                        np.polyfit(
                            np.log(nbel_list[-3:]), np.log(error_list[k, i, -3:]), 1
                        )[0],
                        1,
                    )
                    annotation.slope_marker(
                        (nbel_list[-2], error_list[k, i, -2]),
                        slope,
        ...
        ax.semilogy([], [], color="k", **CONFIGLINE_IGA, label="Gauss quadrature")
        ax.semilogy([], [], color="k", **CONFIGLINE_WQ, label="Weighted quadrature")

        ax.legend(title=r"Degree $p=1,\ldots,3$")
        fig.tight_layout()
