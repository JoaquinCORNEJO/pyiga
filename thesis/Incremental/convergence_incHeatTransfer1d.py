...
def simulate_ht(degree, cuts, nbel_time, quad_args, ivp="alpha", alpha=0.5):
    ...
    if ivp == "alpha":
        ...
        problem.solve_heat_transfer(
            temperature, external_heat_source, time_inc, alpha=alpha
        )
        ...
        args = problem._verify_fun_args(
            {"temperature": np.ones(problem.part.nbqp_total)}
        )
        prop = problem.material.capacity(args) * problem.part.det_jac
        capacity = bspline_operations.assemble_scalar_u_v(
            problem.part.quadrule_list, prop, allow_lumping=False
        )
        inv_capacity = np.linalg.pinv(
            capacity.toarray()[np.ix_(free_ctrlpts, free_ctrlpts)]
        )

        def fun(tm, y_input):
            y_extend = np.zeros(problem.part.nbctrlpts_total)
            y_extend[free_ctrlpts] = y_input
            y_interp = problem.interpolate_temperature(y_extend)
            args = problem._verify_fun_args({"time": tm, "temperature": y_interp})
            b = problem.assemble_volumetric_force(power_density, args=args)[
                free_ctrlpts
            ]
