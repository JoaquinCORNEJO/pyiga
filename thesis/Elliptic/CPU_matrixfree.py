	...
	for quadrule, quadtype in zip(['gs', 'wq', 'wq'], ['leg', 2, 1]):
		...
		for i, degree in enumerate(degree_list):
			...
			if quad_args['quadrule'] == 'wq':
				quadrature = weighted_quadrature(degree, knotvector, quad_args=quad_args)
			...
			nbctrlpts_total = np.product(np.array([quadrature.nbctrlpts for i in range(NDIM)]))
			...
			if quadrule != 'gs' or degree < 11:
				...
				bspline_operations.compute_mf_scalar_u_v([quadrature]*NDIM, prop, np.random.random(nbctrlpts_total), allow_lumping=False)
				finish = time.process_time()
				print('Time Capacity:%.2e' %(finish-start))
				timeMF_capacity[i, 1] = finish - start
				np.savetxt(f"{FOLDER2DATA}MF_capacity_{nbel:d}_{quadrule}_{str(quadtype)}.dat", timeMF_capacity)

			if quadrule != 'gs' or degree < 7:

				prop = np.ones((NDIM, NDIM, nbqp_total))
				start = time.process_time()
				bspline_operations.compute_mf_scalar_gradu_gradv([quadrature]*NDIM, prop, np.random.random(nbctrlpts_total))
				finish = time.process_time()
				print('Time Conductivity:%.2e' %(finish-start))
				timeMF_conductivity[i, 1] = finish - start
				np.savetxt(f"{FOLDER2DATA}MF_conductivity_{nbel:d}_{quadrule}_{str(quadtype)}.dat", timeMF_conductivity)
				...
				start = time.process_time()
				for k in range(NDIM):
					array_out[k, :] = sum(bspline_operations.compute_mf_scalar_gradu_gradv([quadrature]*NDIM,
																						prop, array_in[j, :])
																						for j in range(NDIM))
				...
				np.savetxt(f"{FOLDER2DATA}MF_stiffness_{nbel:d}_{quadrule}_{str(quadtype)}.dat", timeMF_stiffness)

FILENAME = 'MF_stiffness_64'
sufix_list = ['gs_leg', 'wq_2']
label_list = ['MF-Gauss quad.', 'MF-Weighted quad.']
plotoptions = [CONFIGLINE_IGA, CONFIGLINE_WQ]

xx = np.arange(1, 10)
yy_1 = 0.5*xx**3+0.02*xx**4
yy_2 = 1.25*xx

# Load data
fig, ax = plt.subplots(figsize=(5, 5.5))
for label, sufix, plotops in zip(label_list, sufix_list, plotoptions):
	file = np.loadtxt(f'{FOLDER2DATA}{FILENAME}_{sufix}.dat')
	degree_list = file[:, 0]; time_elapsed = file[:, 1]
	ax.semilogy(degree_list, time_elapsed, label=label, **plotops)

ax.semilogy(xx, yy_1, '--', label='$O(Np^4)$', c='tab:gray', alpha=1)
ax.semilogy(xx, yy_2, '--', label='$O(Np)$', c='tab:gray', alpha=0.5)
ax.minorticks_off()
...
for label, sufix, plotops, annot in zip(label_list, sufix_list, plotoptions, annotation):
	...
	ax.text(
		degree_list[-1], 1.5*time_elapsed[-1],
		f'{annot}',
		fontsize=12,
		ha='center', va='bottom',
