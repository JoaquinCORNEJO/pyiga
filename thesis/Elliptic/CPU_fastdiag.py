        ...
        for i, cuts in enumerate(cut_list):
            for j, degree in enumerate(deg_list):
                ...
                fastdiag.compute_space_eigendecomposition(
                    [quadrature] * NDIM, boundary.table_dirichlet
                )
                ...
                print(
                    "For p = %s, nbel = %s, time: %.4f" % (degree, nbel, time_elapsed)
                )
...
for i, TYPESIMU in enumerate(["heat", "meca"]):
    ...
    slope = round(
        np.polyfit(np.log(nbel_list[1:] ** 3), np.log(time_elapsed[1:, 2]), 1)[0], 1
    )
    annotation.slope_marker(
        (nbel_list[-2] ** 3, time_elapsed[-2, 2]),
        slope,
    ...
    tick_locs = 1 + (np.arange(len(deg_list)) + 0.5) * (len(deg_list) - 1) / len(
        deg_list
    )
    ...
    ax.set_xticklabels(
        [r"$10^3$", r"$32^3$", r"$64^3$", r"$128^3$", r"$256^3$", r"$10^8$"]
    )
