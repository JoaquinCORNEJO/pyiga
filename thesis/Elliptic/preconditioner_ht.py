...
for j, [preconditioner, labelfig] in enumerate(
    zip(
        ["WP", "FD", "ILU", "JM"],
        [
            "w.o. preconditioner",
...
ax.legend(ncol=2, bbox_to_anchor=(0.5, 1.25), loc="upper center")
ax.set_ylabel("Relative residue")
ax.set_xlabel("Number of iterations (GMRES)")
ax.set_ylim([1e-12, 1e1])
ax.set_xlim([0, 100])
fig.tight_layout()
fig.savefig(f"{FOLDER2RESU}performance_preconditioner.pdf", bbox_inches="tight")
