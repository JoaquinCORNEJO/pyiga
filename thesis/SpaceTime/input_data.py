...
def nonlineardersfunc(args: dict):
    temperature = args.get("temperature")
    if ISLINEAR:
        Kprop = np.zeros(shape=np.shape(temperature))
    else:
        Kprop = (2.0 / 50) / (np.cosh(temperature / 50)) ** 2
    return Kprop
...
def exactTemperatureRing_spt(args):
    ...
    nc_tm = np.size(time)
    u = np.zeros((nc_sp, nc_tm))
    for i in range(nc_tm):
        u[:, i] = exactTemperatureRing_inc(args={"time": time[i], "position": position})
...
def powerDensityRing_inc(args: dict):
    ...
    u1 = pi * (x**2 + y**2 - 1 / 16)
    u2 = x**2 + y**2 - 1
    u3 = (3 * cos((3 * pi * t) / 2)) / 4 + 1
    u4 = sin((pi * t) / 2)
    if ISLINEAR:
        f = (
            2 * CST * pi * cos(pi * x * y) * sin(u1) * tanh(u2) * u4 * (u3)
        ...
        f = (
            (
                2 * tanh((CST * sin(pi * x * y) * sin(u1) * tanh(u2) * u4 * (u3)) / 50)
                - 3
            )
            * (
                2 * CST * sin(pi * x * y) * sin(u1) * u4 * (tanh(u2) ** 2 - 1) * (u3)
            ...
            - 2
            * (
                tanh((CST * sin(pi * x * y) * sin(u1) * tanh(u2) * u4 * (u3)) / 50)
                - 3 / 2
            )
            * (
                CST * pi * cos(pi * x * y) * sin(u1) * tanh(u2) * u4 * (u3)
                - 2
                * CST
                * x**2
                * pi
            ...
            + (
                4 * tanh((CST * sin(pi * x * y) * sin(u1) * tanh(u2) * u4 * (u3)) / 50)
                - 6
            )
            * (
                2 * CST * sin(pi * x * y) * sin(u1) * u4 * (tanh(u2) ** 2 - 1) * (u3)
            ...
            + 2
            * (
                tanh((CST * sin(pi * x * y) * sin(u1) * tanh(u2) * u4 * (u3)) / 50) ** 2
                - 1
            )
            * (
                2 * CST * x * pi * sin(pi * x * y) * cos(u1) * tanh(u2) * u4 * (u3)
                - 2
                * CST
                * x
                * sin(pi * x * y)
                ...
                * (tanh(u2) ** 2 - 1)
                * (u3)
                + CST * y * pi * cos(pi * x * y) * sin(u1) * tanh(u2) * u4 * (u3)
            )
            * (
                (CST * x * pi * sin(pi * x * y) * cos(u1) * tanh(u2) * u4 * (u3)) / 25
                - (
                    CST
                    * x
                    * sin(pi * x * y)
            ...
            * (
                (CST * x * pi * cos(pi * x * y) * sin(u1) * tanh(u2) * u4 * (u3)) / 50
                - (
                    CST
                    * y
                    * sin(pi * x * y)
            ...
            * (
                CST * x * pi * cos(pi * x * y) * sin(u1) * tanh(u2) * u4 * (u3)
                - 2
                * CST
                * y
                * sin(pi * x * y)
...
def powerDensityRing_spt(args: dict):
    ...
    for i in range(nc_tm):
        f[:, i] = powerDensityRing_inc(args={"time": time[i], "position": position})
...
def simulate_spacetime(
    ...
    time_interval = mygeomdl(
        {"name": "line", "degree": degree_time, "nbel": nbel_time}
    ).export_geometry()
