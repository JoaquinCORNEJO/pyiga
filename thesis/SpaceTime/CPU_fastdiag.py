...
if RUNSIMU:
    ...
    for i, cuts in enumerate(cut_list):
        for j, degree in enumerate(deg_list):
            ...
            boundary_sp = boundary_condition(
                nbctrlpts=np.array([nctrlpts] * NDIM), nb_vars_per_ctrlpt=1
            )
            boundary_sp.add_constraint(
                location_list=[
                    {
                        "direction": ",".join(str(i) for i in range(NDIM)),
            ...
            boundary_sptm = boundary_condition(
                nbctrlpts=np.array([nctrlpts] * (NDIM + 1)), nb_vars_per_ctrlpt=1
            )
            boundary_sptm.add_constraint(
                location_list=[
                    {
                        "direction": ",".join(str(i) for i in range(NDIM + 1)),
                        "face": ",".join(
                            "both" if i != NDIM else "left" for i in range(NDIM + 1)
                        ),
                    }
                ],
                constraint_type="dirichlet",
            ...
            if cuts != cut_list[-1] or degree < 2:
                ...
                fastdiag.compute_space_eigendecomposition(
                    [quadrature] * NDIM, boundary_sp.table_dirichlet
                )
