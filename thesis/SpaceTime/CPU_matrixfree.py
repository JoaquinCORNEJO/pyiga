...
if RUNSIMU:
    ...
    for quadrule, quadtype in zip(["gs", "wq", "wq"], ["leg", 2, 1]):
        ...
        for i, degree in enumerate(deg_list):
            ...
            # Create model
            geometry = mygeomdl(
                {"name": "VB", "degree": degree, "nbel": nbel}
            ).export_geometry()
            ...
            problem = st_heat_transfer_problem(
                material, space_patch, time_patch, boundary
            )
            array_in = np.random.random(
                space_patch.nbctrlpts_total * time_patch.nbctrlpts_total
            )
            ...
            np.savetxt(
                f"{FOLDER2DATA}sptMF_Mass_{quadrule}_{str(quadtype)}.dat", timeMF_mass
            )
            np.savetxt(
                f"{FOLDER2DATA}sptMF_Stiff_{quadrule}_{str(quadtype)}.dat", timeMF_stiff
            )
            np.savetxt(
                f"{FOLDER2DATA}sptMF_MassDers_{quadrule}_{str(quadtype)}.dat",
                timeMF_mass_ders,
            )
            np.savetxt(
                f"{FOLDER2DATA}sptMF_StiffDers_{quadrule}_{str(quadtype)}.dat",
                timeMF_stiff_ders,
            )

fig, ax = plt.subplots(figsize=(5, 5))
plotoptions = [CONFIGLINE_IGA, CONFIGLINE_WQ]
sufixList = ["gs_leg", "wq_1"]
...
for sufix, plotops in zip(sufixList, plotoptions):
    ...
    deg_list = file_M1[:, 0]
    time_list = [
        file_M1[:, 1] + file_K1[:, 1] + file_M2[:, 1] + file_K2[:, 1],
        file_M1[:, 1] + file_K1[:, 1],
    ]
