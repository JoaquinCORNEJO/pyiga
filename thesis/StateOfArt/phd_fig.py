...
def plot_2d_geometry(patch: singlepatch, sample_size=101, plotaxis=True):
    ...
    evalpts = patch.interpolate_field(
        knots_list=[np.linspace(0, 1, sample_size) for _ in range(patch.ndim)]
    )[1]
    u_knots = patch.interpolate_field(
        knots_list=[np.unique(knotvector) for knotvector in patch.knotvector]
    )[1]
    ...
    ax.plot(
        u_knots[0, :], u_knots[1, :], color="k", marker="s", linestyle="", label="Knots"
    )

    ax.set_xticks(np.arange(0, np.ceil(max(evalpts[:, 0])) + 2, 0.5))
    ax.set_yticks(np.arange(0, np.ceil(max(evalpts[:, 1])) + 1, 0.5))
...
if FIGCASE == 0:  # B-spline curve
    ...
    def case0(folder, extension, refinement=2):
        ...
        if refinement == 1:
            ...
            newknots = [
                (uniqueKV[i] + uniqueKV[i + 1]) / 2 for i in range(0, len(uniqueKV) - 1)
            ]
            for knot in newknots:
                crv.insert_knot(knot)
    ...
    for ii in range(3):
        case0(FOLDER2RESU, EXTENSION, refinement=ii)
    ...
    def case1(folder, extension, addDers=True):
        ...
        if addDers:
            ...
            ax.plot(
                quadRule._unique_kv,
                np.zeros(len(quadRule._unique_kv)),
                color="k",
                marker="s",
                linestyle="",
                label="Knots",
    ...
    for addDers in [True, False]:
        case1(FOLDER2RESU, EXTENSION, addDers=addDers)
    ...
    def case2(folder, extension, is2D=True):
        ...
        if is2D:
            ...
            im = axs[1, 0].pcolormesh(
                X, Y, Z, cmap="GnBu", shading="gouraud", rasterized=True
            )
    ...
    for is2D in [True, False]:
        case2(FOLDER2RESU, EXTENSION, is2D=is2D)
    ...
    def case7(folder, extension):
        ...
        for degree, color, label in zip(
            [5, 1], ["tab:green", "tab:blue"], ["Quintic", "Linear"]
        ):
            nbel = 8
            knotvector = create_uniform_knotvector(degree, nbel)
            quadrature = weighted_quadrature(degree, knotvector, {"type": 2})
        ...
        ax.plot(
            ...
            np.zeros_like(quadrature._unique_kv),
            color="k",
            marker="s",
            linestyle="None",
            markersize=3,
