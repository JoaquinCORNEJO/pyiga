...
def simulate_incremental(degree, cuts, nbel_time):
	...
	# Create geometry
	geometry = mygeomdl({'name':'line', 'degree':degree, 'nbel':int(2**cuts)}).export_geometry()
	...
	for i, t in enumerate(time_patch):
		external_heat_source[:, i] = problem.assemble_volumetric_force(power_density_inc, args={'time':t})

	# Solve problem
	problem._tolerance_nonlinear = 1e-3
	problem.solve_heat_transfer(temperature, external_heat_source, time_patch, alpha=0.5)
...
def simulate_spacetime(degree, cuts, nbel_time,	auto_inner_tolerance=True):
	...
	geometry = mygeomdl({'name':'line', 'degree':degree, 'nbel':int(2**cuts)}).export_geometry()
	space_patch = singlepatch(geometry, quad_args={'quadrule':'wq', 'type':2})
	time_interval = mygeomdl({'name':'line', 'degree':1, 'nbel':nbel_time}).export_geometry()
	...
	problem_spt.solve_heat_transfer(temperature, external_force, use_picard=True,
