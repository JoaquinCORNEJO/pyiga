...
if VIDCASE == 0:  # B-spline curve
	...
	def simulate(degree, nbel, nbel_time=None):
		...
		geo_parameters = {'name': 'TP', 'degree': degree, 'nbel': nbel, 
						'geo_parameters':{'XY':np.array([[0.0, 0.0], [10.0, 0.0], 
														[10.0, 5.0], [0.0, 5.0]])}}
		geometry = mygeomdl(geo_parameters).export_geometry()
		patch = singlepatch(geometry, quad_args={'quadrule':'gs', 'type':'leg'})

		if nbel_time is None: nbel_time = np.copy(nbel)
		timespan = 20
		...
		boundary_inc.add_constraint(location_list=[{'direction':'y', 'face':'top'}], 
		...
		# Solve
		problem_inc.solve_heat_transfer(temperature, external_force, time_list=time_inc, alpha=0.5)
	...
	for k, i in enumerate(range(0, np.size(output, axis=1), 3)):
		problem_inc.part.postprocessing_primal(fields={'temp':output[:, i]}, 
										name=f'out_{k}', folder=FOLDER2RESU)
	
	import imageio
	images = []
	for i in range(14): 
		filepath = vtk2png(filename=f'out_{i}', folder=FOLDER2RESU, title='Temperature', 
					camera_position='xy', position_y=0.2, clim=[0, 1],)
