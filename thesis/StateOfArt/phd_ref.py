...
if FIGCASE == 0:
	...
	ax.semilogy(table_literature.degree[:-1], table_literature.standard[:-1], **CONFIGLINE_IGA,
				label='Element-loop w. \nGauss quadrature')
	ax.semilogy(table_literature.degree[:-1], table_literature.weighted[:-1], **CONFIGLINE_WQ,
				label='Row-loop w. \nWeighted quadrature')
		
	for xx, yy, text in zip([table_literature.degree[7], table_literature.degree[7]], 
							[table_literature.standard[7], table_literature.weighted[7]], 
							['26 h', '20.5 s']):
		ax.text(
			xx, 1.5*yy,
			f'{text}',
	...
	label_list = ['FE radial return', f'ST-IGA deg. {2:d}', f'ST-IGA deg. {3:d}']
	...
	for pos in range(1, len(error_file.columns)):
		im = ax.scatter(time_file.iloc[:, pos], error_file.iloc[:, pos],
					c=degree_list, cmap=cmap, marker=plotops['marker'], s=15*plotops['markersize'])

		ax.loglog(time_file.iloc[:, pos], error_file.iloc[:, pos],
				color='k', marker='', linestyle=plotops['linestyle'], alpha=0.5)

		if pos>1:
			ax.text(time_file.iloc[-1, pos]/2, error_file.iloc[-1, pos]/40, str(int(2**(pos+3)))+r'$^3$'+' el.')
		else:
			ax.text(time_file.iloc[int(len(degree_list)/2), pos]/2, error_file.iloc[-1, pos]/40, str(int(2**(pos+3)))+r'$^3$'+' el.')
