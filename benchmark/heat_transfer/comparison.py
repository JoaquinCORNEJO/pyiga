...
time_interval = mygeomdl(
    {"name": "line", "degree": 1, "nbel": nbel, "geo_parameters": {"L": 1.0}}
).export_geometry()
...
problem_sptm.solve_heat_transfer(
    temperature_sptm, external_heat_source_sptm, auto_inner_tolerance=False
)
finish = process_time()
time_elapsed_sptm = finish - start
rel_error_sptm = problem_sptm.norm_of_error(
    temperature_sptm, norm_args={"type": "L2", "exact_function": exact_temperature_sptm}
)[-1]
...
for i, t in enumerate(time_list):
    external_heat_source_inc[:, i] = problem_inc.assemble_volumetric_force(
        power_density_inc, args={"time": t}
    )

temperature_inc = np.zeros_like(external_heat_source_inc)
problem_inc.solve_heat_transfer(
    temperature_inc, external_heat_source_inc, time_list, alpha=0.5
)
finish = process_time()
time_elapsed_inc = finish - start
rel_error_inc = problem_sptm.norm_of_error(
    temperature_inc.ravel("F"),
    norm_args={"type": "L2", "exact_function": exact_temperature_sptm},
)[-1]
...
print(
    f"Forr incremental method: error {rel_error_inc:.2e} and time {time_elapsed_inc:.2f}"
)
...
diff_temp = bspline_operations.interpolate_meshgrid(
    quadrule_list=space_patch.quadrule_list,
    knots_list=[knots_interp],
    u_ctrlpts=temperature_inc.T
    - np.reshape(temperature_sptm, newshape=(-1, len(time_list)), order="F").T,
)
