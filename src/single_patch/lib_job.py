...
class problem:
	def __init__(
		...
		self.part = patch
		self.nbvars = boundary.nbvars
		self.sp_free_ctrlpts, self.sp_constraint_ctrlpts = (
			boundary.select_nodes4solving()
		)
		self.sp_table_dirichlet = boundary.table_dirichlet
		self._sp_boundary = boundary
		self._linear_tolerance_list = []
		self._nonlinear_rate_list = []
		self._linear_residual_list = []
		self._nonlinear_residual_list = []
		self._solution_history_list = {}
		return

	def _add_solver_parameters(self, args: dict):
		self._linear_solver = args.get("solver", "GMRES")
		self._maxiters_linear = args.get("iters_linear", 100)
		self._tolerance_linear = args.get("tol_linear", 1e-8)
		self._maxiters_nonlinear = args.get("iters_nonlinear", 10)
		self._tolerance_nonlinear = args.get("tol_nonlinear", 1e-6)
		self._use_preconditioner = args.get("use_preconditioner", True)
		self._update_preconditioner = args.get("update_preconditioner", True)
	...
	def _solve_linear_system(
		...
		solv = solver(max_iters=max_iters, tolerance=tolerance)
		...
		if solve_method:
			if not self._use_preconditioner:
				Pfun = None
			output = solve_method(
				Afun, b, Pfun=Pfun, dotfun=dotfun, cleanfun=cleanfun, dod=dod, args=args
			)
		else:
			raise ValueError(f"Unknown linear solver method: {self._linear_solver}")
		return output

	def _calculate_norm(self, u, uders, det_jac, parametric_weights, norm_type="l2"):
		ndim = len(parametric_weights)
		shape = [len(wgt) for wgt in parametric_weights]
		u2_l2, u2_sh1 = 0.0, 0.0
		if norm_type in ["l2", "h1"]:
			u2_l2 += np.sum(u**2, axis=0)
		if norm_type in ["h1", "semih1"]:
			u2_sh1 += np.sum(uders**2, axis=(0, 1))

		tmp = np.reshape((u2_l2 + u2_sh1) * det_jac, tuple(shape), order="F")
		subs = ",".join("abcdef"[:ndim]) + "," + "abcdef"[:ndim]
		norm2 = np.einsum(subs + "->", *parametric_weights, tmp)
	...
	def _compute_exact_values_for_norm(
		...
		# Second method
		part_ref = norm_args.get("part_ref", None)
		time_ref = norm_args.get("time_ref", None)
		u_ref = norm_args.get("u_ref", None)
		if (
			allow_spacetime
			...
			u_exact, uders_exact = prepare_norm_computation(
				...
				time_ref,
				u_ref,
				knots_ref=parametric_position,
				norm_type=norm_type,
			)[:2]
		elif isinstance(part_ref, singlepatch):
			assert isinstance(u_ref, np.ndarray), "Solution should be numpy array"
			u_exact, uders_exact = prepare_norm_computation(
				part_ref, u_ref, knots_ref=parametric_position, norm_type=norm_type
			)[:2]
...
class space_problem(problem):
	...
	def _verify_fun_args(self, args):
		if args is None:
			args = {}
		assert isinstance(
			args, dict
		), "allowed extra arguments should be in dictionnary"
		args["position"] = self.part.qp_phy
		args["shape_quadpts"] = (self.part.nbqp_total,)
		return args

	def _prepare_surface_force_computation(self, location: dict):
	...
	def assemble_surface_force(self, fun, location: dict, args={}):
	...
	def assemble_volumetric_force(self, fun, args=None):
		...
		if nr == 1: array_out = np.ravel(array_out)
		return array_out

	def _prepare_norm_computation(
		self, part_ref: singlepatch, u_ref, knots_ref, norm_type="l2"
	):
	...
	def norm_of_error(self, u_ctrlpts, norm_args: dict):
		...
		output = self._prepare_norm_computation(
			self.part,
			u_ctrlpts,
			knots_ref=[None for _ in range(self.part.ndim)],
			norm_type=norm_type,
		)
		(
			u_interp,
	...
	def compute_L2projection(self, u_at_quadpts):
...
class spacetime_problem(problem):
	def __init__(
		...
		self.time = time_patch
		self._quadrature_list = self.part.quadrule_list + self.time.quadrule_list
	...
	def _verify_fun_args(self, args):
		...
		args["position"] = self.part.qp_phy
		args["time"] = np.ravel(self.time.qp_phy)
		args["shape_quadpts"] = (self.part.nbqp_total * self.time.nbqp_total,)
		return args
	...
	def _prepare_norm_computation(
		...
		return (
			...
			uders_interp,
			parametric_position,
			[quadpts_phy_space, quadpts_phy_time],
			np.kron(det_jac_time, det_jac_space),
			parametric_weights,
		)

	def norm_of_field(self, u_ctrlpts, norm_type="l2"):
		output = self._prepare_norm_computation(
			self.part,
			self.time,
			u_ctrlpts,
