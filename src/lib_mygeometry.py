...
class mygeomdl():
	...
	def export_geometry(self):
		...
		geometry_map = {
			...
			'quarter_annulus': (2, {'Rin': 0.25, 'Rex': 1.0}, self._create_quarter_annulus),
			'qa': (2, {'Rin': 0.25, 'Rex': 1.0}, self._create_quarter_annulus),
			'square': (2, {'XY': np.array([[0.0, 0.0], [1.0, 0.0], [1.0, 1.0], [0.0, 1.0]])}, self._create_quadrilateral),
			'sq': (2, {'XY': np.array([[0.0, 0.0], [1.0, 0.0], [1.0, 1.0], [0.0, 1.0]])}, self._create_quadrilateral),
			'trapezium': (2, {'XY': np.array([[0.0, -0.5], [0.4, -0.2], [0.4, 0.2], [0.0, 0.5]])}, self._create_quadrilateral),
			'tp': (2, {'XY': np.array([[0.0, -0.5], [0.4, -0.2], [0.4, 0.2], [0.0, 0.5]])}, self._create_quadrilateral),
			'cube': (3, {'Lx': 1.0, 'Ly': 1.0, 'Lz': 1.0}, self._create_parallelepiped),
			'cb': (3, {'Lx': 1.0, 'Ly': 1.0, 'Lz': 1.0}, self._create_parallelepiped),
			'thick_ring': (3, {'Rin': 1.0, 'Rex': 2.0, 'height': 1.0}, self._create_thick_ring),
			'tr': (3, {'Rin': 1.0, 'Rex': 2.0, 'height': 1.0}, self._create_thick_ring),
			'rotated_quarter_annulus': (3, {'Rin': 1.0, 'Rex': 2.0, 'exc': 1.0}, self._create_rotated_quarter_annulus),
			'rqa': (3, {'Rin': 1.0, 'Rex': 2.0, 'exc': 1.0}, self._create_rotated_quarter_annulus),
			'prism': (3, {'XY': np.array([[0.0, -7.5], [6.0, -2.5], [6.0, 2.5], [0.0, 7.5]]), 'height': 1.0}, self._create_prism),
	...
	def _create_quarter_annulus(self, degree_u:int, degree_v:int, nbel_u:int, nbel_v:int, args:dict):
		...
		ctrlpts = [[x_line * x_arc, x_line * y_arc, 0.0] for x_line in ctrlpts_line for x_arc, y_arc in ctrlpts_arc]
	...
	def _create_quadrilateral(self, degree_u:int, degree_v:int, nbel_u:int, nbel_v:int, args:dict):
		...
		ctrlpts = [[ax[0]*xt + ax[1]*yt + ax[2]*xt*yt + ax[3],
					ay[0]*xt + ay[1]*yt + ay[2]*xt*yt + ay[3],
					0.0] for xt in ctrlpts_u for yt in ctrlpts_v]
	...
	def _create_parallelepiped(self, degree_u:int, degree_v:int, degree_w:int, nbel_u:int, nbel_v:int, nbel_w:int, args:dict):
		...
		ctrlpts = [[cptu * Lx, cptv * Ly, cptw * Lz] for cptw in ctrlpts_w for cptu in ctrlpts_u for cptv in ctrlpts_v]
	...
	def _create_rotated_quarter_annulus(self, degree_u:int, degree_v:int, degree_w:int, nbel_u:int, nbel_v:int, nbel_w:int, args:dict):
		...
		# Get control points
		ctrlpts = [
			[x_line * x_arc_1, (x_line * y_arc_1 + exc) * y_arc_2, (x_line * y_arc_1 + exc) * z_arc_2]
