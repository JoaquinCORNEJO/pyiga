...
class singlepatch():
	def __init__(self, obj, quad_args:dict):
		verification_list = [
		...
		self.nbctrlpts = np.array([
			len(self.knotvector[i]) - self.degree[i] - 1 
	...
	def _read_degree(self, obj:Union[BSpline.Curve, BSpline.Surface, BSpline.Volume]):
	...
	def _read_knotvector(self, obj:Union[BSpline.Curve, BSpline.Surface, BSpline.Volume]):
	...
	def _read_control_points(self, obj:Union[BSpline.Curve, BSpline.Surface, BSpline.Volume]):
	...
	def postprocessing_primal(
		...
		if write_file: 
			gridToVTK(f"{folder}{name}", position_data[0], position_data[1], position_data[2], pointData=point_data)
