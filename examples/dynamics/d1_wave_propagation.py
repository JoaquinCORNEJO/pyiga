...
displacement[:, :, 0] = problem.compute_L2projection(compute_initial_displacement({'position': problem.part.qp_phy}))
...
disp_interp_initial = np.ravel(bspline_operations.interpolate_meshgrid(quadrule_list=patch.quadrule_list,
...
disp_interp_final =  np.ravel(bspline_operations.interpolate_meshgrid(quadrule_list=patch.quadrule_list,
															knots_list=[knots_interp], 
...
ax.legend()
fig.savefig(RESULT_FOLDER+'wave_propagation')