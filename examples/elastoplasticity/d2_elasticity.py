...
def surface_force(args: dict):
    ...
    force[0, :] = TRACTION / 2 * (2 * cos_theta - b * (2 * cos_theta + 3 * cos_3theta) + 3 * b2 * cos_3theta)
    force[1, :] = TRACTION / 2 * 3 * sin_3theta * (b2 - b)
    return force

# Create geometry
geo_parameters = {'name': 'QA', 'degree': DEGREE, 'nbel': NBEL, 'geo_parameters':{'Rin': RINT, 'Rex': REXT}}
material = J2plasticity3d({'elastic_modulus':YOUNG, 'elastic_limit':1e10, 'poisson_ratio':POISSON})
...
boundary.add_constraint(location_list=[{'direction':'y', 'face':'top'}, 
									{'direction':'y', 'face':'bottom'}], 
						constraint_type='dirichlet')

# Solve elastic problem
problem = mechanical_problem(material, patch, boundary)
external_force = problem.assemble_surface_force(surface_force, location={'direction':'x', 'face':'right'})
...
problem.part.postprocessing_dual(fields={'vms':vmstress}, name='elasticity', folder=RESULT_FOLDER)