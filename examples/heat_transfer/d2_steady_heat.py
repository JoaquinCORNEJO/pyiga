...
def exact_temperature(args:dict):
	position = args.get('position')
	x = position[0, :]; y = position[1, :]
	u = np.sin(np.pi*x)*np.sin(np.pi*y)*(x**2 + y**2 -1.0)*(x**2 + y**2 - 4.0)
	return u
...
def simulate(degree, nbel, quad_args=None):
	geo_args = {'name':'QA', 'degree':degree, 'nbel':nbel, 
...
problem.part.postprocessing_primal(fields={'temp':temperature}, name='steady', folder=RESULT_FOLDER)
...
for quadrule, quadtype, plotpars in zip(['gs', 'wq', 'wq'], ['leg', 1, 2], [gauss_plot, wq1_plot, wq2_plot]):
	quad_args = {'quadrule': quadrule, 'type': quadtype}
	for i, degree in enumerate(degree_list):
		color = color_list[i]
		for j, cuts in enumerate(cuts_list):
			problem, temperature = simulate(degree, 2**cuts, quad_args)
			_, relerror_list[j] = problem.norm_of_error(temperature, 
																norm_args={'type':'l2',
		...
		ax.loglog(2**cuts_list, relerror_list, label=label, color=color, marker=plotpars['marker'], markerfacecolor='w',
				markersize=plotpars['markersize'], linestyle=plotpars['linestyle'])
		fig.savefig(figname)

ax.loglog([], [], color='k', marker=wq1_plot['marker'], markerfacecolor='w',
			markersize=wq1_plot['markersize'], linestyle=wq1_plot['linestyle'], label="IGA-WQ 1")
ax.loglog([], [], color='k', marker=wq2_plot['marker'], markerfacecolor='w',
			markersize=wq2_plot['markersize'], linestyle=wq2_plot['linestyle'], label="IGA-WQ 2")
